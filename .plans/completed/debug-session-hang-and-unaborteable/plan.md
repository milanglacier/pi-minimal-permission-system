...
- Target the latest Pi behavior only, as requested during implementation. The global Pi CLI, Tau's integration-test dependency, and npm's latest release are all `0.85.1`; no legacy Pi compatibility paths were added. This checkout now declares `@earendil-works/pi-coding-agent@^0.85.1` as a development dependency (added during review, replacing the previously installed `0.83.0`), so typechecking and the behavior tests run against the same Pi line as the runtime validation; the peer range stays `*` because the signal API also exists in earlier releases. The current API explicitly types `ctx.signal` as `AbortSignal | undefined`, so optional access follows the current contract rather than supporting older versions.
